...
#' @param type Column name corresponding to the types of samples (control and treated samples) on pheno data provided to the `import_celfiles` function (optional)
#' @param batch Column name corresponding to each replicates on pheno data provided to the `import_celfiles` function (optional)
  ...
  if (!is.null(filename)) {
    if(is.null(type) && is.null(batch)) {
      ...
      print(ggplot2::ggplot(pca_df, ggplot2::aes_string(x = pca_df[,1], y = pca_df[,2], col = groups)) +
        ggplot2::labs(title = "PCA after normalization", x = paste0("PCA ", components[1], " (", importance[1], "%)"),
             y = paste0("PCA ", components[2], " (", importance[2], "%)"), col = "Group") +
      ...
      print(ggplot2::ggplot(pca_df, ggplot2::aes_string(x = pca_df[,1], y = pca_df[,2], col = groups, shape = type)) +
        ggplot2::labs(title = "PCA after normalization", x = paste0("PCA ", components[1]," (", importance[1], "%)"),
             y = paste0("PCA ", components[2], " (", importance[2], "%)"), col = "Group", shape = "Type") +
        ggplot2::geom_point(size = 2) +
        ggplot2::scale_color_brewer(palette = "Set1", type = "qual") +
        ggplot2::theme_bw())
      dev.off()
